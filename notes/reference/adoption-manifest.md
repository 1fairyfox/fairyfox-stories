...
| ship-contract | gap(phase-2: confirm Scorecard ≥ 7.0 after first run, due 2026-08-08) | 1.6.1 / 2d614f0 | 2026-08-02 compliance pass | **Scorecard wired** (`scorecard.yml`) — the ≥ 7.0 floor becomes measurable after the first `main` run; expected near the solo ceiling (~8). **All open PRs now triaged** (the 5 Dependabot PRs closed with reason, 0 open); no stale dep PRs / deprecation warnings / skipped tests; no-hacks held. Only the numeric Scorecard confirmation remains. |
| supply-chain-hardening | implemented | 1.6.1 / 2d614f0 | 2026-08-02 compliance pass | `dependabot.yml` ✓, `SECURITY.md` ✓, `.gitattributes` ✓; **provenance now attested AND the `.intoto.jsonl` attached as a release asset** (`release.yml` — attestation-alone scores 0, fixed this pass) ✓; SAST `codeql.yml` ✓ + `scorecard.yml` ✓; **every workflow has top-level `permissions: contents: read` + every `uses:` SHA-pinned with a version comment** (pages/release were tag-pinned, fixed) ✓; `main` protection restored with the full CI suite as required checks ✓; releases go through a PR merge ✓. Solo ceiling ~8/10 (Scorecard Code-Review needs an approving human — noted, not chased). |
| dependencies | implemented | 1.6.1 / 2d614f0 | 2026-08-02 compliance pass | Dependabot (github-actions, grouped, weekly → `dev`); zero runtime deps; **all workflow actions bumped to latest majors + SHA-pinned** this pass, clearing the 5 open Dependabot PRs (closed with reason). Local test gate green. |
| repo-hygiene | implemented | 1.6.1 / 2d614f0 | 2026-08-02 compliance pass | `scripts/check-links.mjs` + `scripts/check-tidy.mjs` + **`scripts/check-standards.mjs` (machine-checked invariants: header active-nav, VERSION==newest main tag) wired into `npm test` + CI**; repo **auto-delete-on-merge enabled**; **`dev` deletion/force-push-protected**; branch litter clean. |
...
| ci-secrets | implemented (N-A: none referenced) | 1.6.1 / 2d614f0 | 2026-08-02 compliance pass | **No workflow references any `secrets.*` service token** — nothing renders "unknown"/unauthenticated, so the standard passes as a clean N/A. Scorecard runs on the default token. `SONAR_TOKEN`/`CODECOV_TOKEN` become relevant only if/when the owner wires Codecov/SonarCloud (then set them + uncomment the badges). |
